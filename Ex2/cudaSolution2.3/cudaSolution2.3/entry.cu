...
__global__ void dev_step(float* dev_points, float* dev_ret, float* dev_s) {
	float d0 = dev_points[threadIdx.x + 1] - dev_points[threadIdx.x] + 1e-9;//dj-1.5
	float d1 = dev_points[threadIdx.x + 2] - dev_points[threadIdx.x + 1] + 1e-9;//dj-0.5
	float d2 = dev_points[threadIdx.x + 3] - dev_points[threadIdx.x + 2] + 1e-9;//dj+0.5
	float d3 = dev_points[threadIdx.x + 4] - dev_points[threadIdx.x + 3] + 1e-9;//dj+1.5

	float r0 = d0 / d1;//rj-0.5-
	float r1 = d2 / d1;//rj-0.5+
	float r2 = d1 / d2;//rj+0.5-
	float r3 = d3 / d2;//rj+0.5+

	dev_ret[threadIdx.x + 2] = dev_points[threadIdx.x + 2] - *dev_s * (dev_points[threadIdx.x + 2] - dev_points[threadIdx.x + 1])\
		- 0.5 * (1 - *dev_s) * *dev_s * (minmod(r2, r3) * d2 - minmod(r0, r1) * d1);
}
...
__global__ void dev_step_1st_and_last(float* dev_points, float* dev_ret, float* dev_s, int last) {
	...
	dev_ret[last] = dev_points[last] - *dev_s * (dev_points[last] - dev_points[last - 1]);
	dev_ret[last-1] = dev_points[last-1] - *dev_s * (dev_points[last - 1] - dev_points[last - 2]);
}
...
float solver::step()
{
	...
	dev_step <<<1, this->nums - 4 >>> (this->dev_points, this->dev_ret, this->dev_s);
